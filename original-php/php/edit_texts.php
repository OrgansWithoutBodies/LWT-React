...
if (isset($_REQUEST['del'])) {
	$message3 = runsql(
		'delete from ' . $tbpref . 'textitems where TiTxID = ' . $_REQUEST['del'],
		"Text items deleted"
	);
	$message2 = runsql(
...
elseif (isset($_REQUEST['arch'])) {
	$message3 = runsql(
		'delete from ' . $tbpref . 'textitems where TiTxID = ' . $_REQUEST['arch'],
		"Text items deleted"
	);
	$message2 = runsql(
...
elseif (isset($_REQUEST['op'])) {
	...
	if (strlen(prepare_textdata($_REQUEST['TxText'])) > 65000) {
		...
		$message3 = runsql(
			'delete from ' . $tbpref . 'textitems where TiTxID = ' . $id,
			"Textitems deleted"
		);
		adjust_autoincr('sentences', 'SeID');
		adjust_autoincr('textitems', 'TiID');
...
if (isset($_REQUEST['new'])) {
	...
	<h4>New Text <a target="_blank" href="info.htm#howtotext"><img src="icn/question-frame.png" title="Help"
				alt="Help" /></a> </h4>
	...
	<form class="validate" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
		<table class="tab3" cellspacing="0" cellpadding="5">
			...
			<tr>
				...
				<td class="td1"><input type="text" class="notempty checkoutsidebmp" data_info="Title" name="TxTitle"
						value="" maxlength="200" size="60" /> <img src="icn/status-busy.png" title="Field must not be empty"
						alt="Field must not be empty" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="td1"><input type="text" class="checkurl checkoutsidebmp" data_info="Source URI"
						name="TxSourceURI" value="" maxlength="1000" size="60" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="td1"><input type="text" class="checkoutsidebmp" data_info="Audio-URI" name="TxAudioURI" value=""
	...
	<p class="smallgray">Import of a <b>long text</b>, without audio, with splitting it up into smaller texts:</p>
	<p><input type="button" value="Long Text Import" onclick="location.href='long_text_import.php';" /> </p>
...
elseif (isset($_REQUEST['chg'])) {
	...
	if ($record = mysqli_fetch_assoc($res)) {
		...
		<h4>Edit Text <a target="_blank" href="info.htm#howtotext"><img src="icn/question-frame.png" title="Help"
					alt="Help" /></a></h4>
		<script type="text/javascript" src="js/unloadformcheck.js" charset="utf-8"></script>
		<form class="validate" action="<?php echo $_SERVER['PHP_SELF']; ?>#rec<?php echo $_REQUEST['chg']; ?>" method="post">
			...
			<table class="tab3" cellspacing="0" cellpadding="5">
				...
				<tr>
					<td class="td1 right">Title:</td>
					<td class="td1"><input type="text" class="notempty checkoutsidebmp" data_info="Title" name="TxTitle"
							value="<?php echo tohtml($record['TxTitle']); ?>" maxlength="200" size="60" /> <img
							src="icn/status-busy.png" title="Field must not be empty" alt="Field must not be empty" /></td>
				</tr>
				<tr>
					<td class="td1 right">Text:<br /><br />(max.<br />65,000<br />bytes)</td>
					<td class="td1">
						<textarea <?php echo getScriptDirectionTag($record['TxLgID']); ?> name="TxText"
							class="notempty checkbytes checkoutsidebmp" data_maxlength="65000" data_info="Text" cols="60"
							rows="20"><?php echo tohtml($record['TxText']); ?></textarea> <img src="icn/status-busy.png"
							title="Field must not be empty" alt="Field must not be empty" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="td1 right">Source URI:</td>
					<td class="td1"><input type="text" class="checkurl checkoutsidebmp" data_info="Source URI"
							name="TxSourceURI" value="<?php echo tohtml($record['TxSourceURI']); ?>" maxlength="1000"
							size="60" /></td>
				</tr>
				<tr>
					<td class="td1 right">Tags:</td>
				...
				<tr>
					<td class="td1 right">Audio-URI:</td>
					<td class="td1"><input type="text" class="checkoutsidebmp" data_info="Audio-URI" name="TxAudioURI"
							value="<?php echo tohtml($record['TxAudioURI']); ?>" maxlength="200" size="60" />
						<span id="mediaselect">
							<?php echo selectmediapath('TxAudioURI'); ?>
						</span>
					</td>
...
// DISPLAY
	...
	<p>
		<a href="<?php echo $_SERVER['PHP_SELF']; ?>?new=1"><img src="icn/plus-button.png" title="New" alt="New" /> New Text
			...</a> &nbsp; | &nbsp;
		<a href="long_text_import.php"><img src="icn/plus-button.png" title="Long Text Import" alt="Long Text Import" />
			Long Text Import ...</a>
	</p>
	...
	<form name="form1" action="#" onsubmit="document.form1.querybutton.click(); return false;">
		<table class="tab1" cellspacing="0" cellpadding="5">
			...
			<tr>
				...
				<td class="td1 center" colspan="2">
					Text Title (Wildc.=*):
					<input type="text" name="query" value="<?php echo tohtml($currentquery); ?>" maxlength="50"
						size="15" />&nbsp;
					<input type="button" name="querybutton" value="Filter"
						onclick="{val=document.form1.query.value; location.href='edit_texts.php?page=1&amp;query=' + val;}" />&nbsp;
	...
	if ($recno == 0) {
		...
		<form name="form2" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
			...
			<table class="tab1" cellspacing="0" cellpadding="5">
				<tr>
					<th class="th1" colspan="2">Multi Actions <img src="icn/lightning.png" title="Multi Actions"
							alt="Multi Actions" /></th>
				</tr>
				<tr>
					<td class="td1 center">
					...
					<td class="td1 center">
						Marked Texts:&nbsp;
						<select name="markaction" id="markaction" disabled="disabled"
			...
			<table class="sortable tab1" cellspacing="0" cellpadding="5">
				<tr>
					...
					<th class="th1 clickable">Title [Tags] / Audio:&nbsp;<img src="icn/speaker-volume.png" title="With Audio"
							alt="With Audio" />, Src.Link:&nbsp;<img src="icn/chain.png" title="Source Link available"
							alt="Source Link available" />, Ann.Text:&nbsp;<img src="icn/tick.png"
							title="Annotated Text available" alt="Annotated Text available" /></th>
	...
	<p><input type="button" value="Archived Texts" onclick="location.href='edit_archivedtexts.php?query=&amp;page=1';" />
	</p>
