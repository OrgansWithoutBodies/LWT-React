...
export function serializeJsonToSQL(
  ...
  const serializedData = Object.fromEntries(
    Object.keys(data).map((key) => [
      ...
      data[key as keyof typeof data].map((val) => Object.fromEntries(
        (Object.keys(val) as (keyof typeof val)[]).map((entryKey) => (val[entryKey] === null
          ? [entryKey, 'NULL']
  ...
  ${serializedData.archivedtexts
    .map((entry) => `  INSERT INTO archivedtexts VALUES(${entry.AtID},${entry.AtLgID},${entry.AtTitle},${entry.AtText},${entry.AtAnnotatedText},${entry.AtAudioURI},${entry.AtSourceURI});\n`)
    .join('')}
...
${serializedData.archtexttags
    .map((entry) => `  INSERT INTO archtexttags VALUES(${entry.AgAtID},${entry.AgT2ID});\n`)
    .join('')}
...
${serializedData.sentences
    .map((entry) => `  INSERT INTO wordtags VALUES(
...
${serializedData.settings
    .map((entry) => `  INSERT INTO settings VALUES(${entry.StKey},${entry.StValue});\n`)
    .join('')}
...
${serializedData.tags
    .map((entry) => `  INSERT INTO tags VALUES(${entry.TgID},${entry.TgText}${entry.TgComment});\n`)
    .join('')}
...
${serializedData.tags2
    .map((entry) => `  INSERT INTO tags2 VALUES(${entry.T2ID},${entry.T2Text}${entry.T2Comment});\n`)
    .join('')}
...
${serializedData.textitems
    .map((entry) => `  INSERT INTO textitems VALUES(${entry.TiID},${entry.TiLgID},${entry.TiTxID},${entry.TiSeID},${entry.TiOrder},${entry.TiWordCount},${entry.TiText},${entry.TiTextLC},${entry.TiIsNotWord});\n`)
    .join('')}
...
${serializedData.texts
    .map((entry) => `  INSERT INTO texts VALUES(${entry.TxID},${entry.TxLgID},${entry.TxTitle},${entry.TxText},${entry.TxAnnotatedText},${entry.TxAudioURI},${entry.TxSourceURI});\n`)
    .join('')}
...
${serializedData.texttags
    .map((entry) => `  INSERT INTO texttags VALUES(${entry.TtTxID},${entry.TtT2ID});\n`)
    .join('')}
...
${serializedData.words
    .map((entry) => `  INSERT INTO words VALUES(${entry.WoID},${entry.WoLgID},${entry.WoText},${entry.WoTextLC},${entry.WoStatus},${entry.WoTranslation},${entry.WoRomanization},${entry.WoSentence},${entry.WoCreated},${entry.WoStatusChanged},${entry.WoTodayScore},${entry.WoTomorrowScore},${entry.WoRandom});\n`)
    .join('')}
...
${serializedData.wordtags
    .map((entry) => `  INSERT INTO wordtags VALUES(${entry.WtWoID},${entry.WtTgID});\n`)
    .join('')}`;
